...
int manage_speak( struct chat *tran_msg, int confd)
{
	...
	temp_user = find_online(tran_msg->f_name);
	
	if(temp_user == NULL)
	{
		strcpy(tran_msg->m_name,"Server\n");
		strcpy(tran_msg->msg, "The user does not exist\n");
		
		n_write = Write(confd,tran_msg,sizeof(struct chat));

		if(n_write <= 0)
		{
			fputs("manage_speak: write error\n",stderr);
			temp_user = find_by_confd(confd);	
			delete_online(temp_user);
			notice(tran_msg->m_name,OFFLINE);
			close(confd);
			return -1;
		}
		return 0;
	...
	debug_msg("admin:f_name %s",tran_msg->f_name);
	temp_user->speak_flag = 0;

	strcpy(tran_msg->m_name,"Server\n");
	strcpy(tran_msg->msg,"You are fobideed to speaking\n");
	...
	strcpy(tran_msg->m_name,"Server\n");
	strcpy(tran_msg->msg,"Set succefully!\n");
...
int kick_one(struct chat *tran_msg, int confd)
{
	...
	fputs("kick_one:Write faliure\n", stderr);
        ...
        delete_online(temp);
	close(temp->confd);
	notice(temp->user_msg.name, OFFLINE);
	return -1;
    }
